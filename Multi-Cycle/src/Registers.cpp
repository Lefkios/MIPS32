...
Registers::Registers() {
    ...
    // Initialize temporary registers to 0
    IR = "";
    A = 0;
    B = 0;
    ALUOut = 0;
...
// Temporary register functions

void Registers::setIR(const std::string& instruction) {
     IR = instruction;
}

std::string Registers::getIR() const { return IR; }

void Registers::setA(int value) {
     A = value;
}

int Registers::getA() const {
     return A;
}

void Registers::setB(int value) {
    B = value;
}

int Registers::getB() const {
     return B;
}

void Registers::setALUOut(int value) {
    ALUOut = value;
}
int Registers::getALUOut() const {
     return ALUOut;
}

void Registers::setMDR(int value) {
    MDR = value;
}

int Registers::getMDR() const {
    return MDR;
}
